...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    // Memuat Kode VIP Saat Ini
    const unsubVip = onSnapshot(doc(db, "pengaturan", "vip"), (docSnap) => {
      if (docSnap.exists()) {
        setKodeVipAktif(docSnap.data().kode);
      } else {
        setKodeVipAktif("Belum Diatur!");
      }
    });
  ...
  const simpanMateri = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await addDoc(collection(db, "materi_belajar"), { judul, mapel, format, link, tanggal: new Date().toISOString() });
      setJudul(""); setLink("");
  ...
  return (
    <main className="min-h-screen bg-sky-50 p-6 font-sans">
      <nav className="max-w-6xl mx-auto flex justify-between items-center bg-white border-4 border-slate-900 p-4 rounded-2xl shadow-[6px_6px_0_0_rgba(15,23,42,1)] mb-8">
        ...
        <button className="bg-red-400 hover:bg-red-500 text-slate-900 font-bold px-5 py-2 rounded-xl border-4 border-slate-900 shadow-[4px_4px_0_0_rgba(15,23,42,1)] active:shadow-none active:translate-y-1 active:translate-x-1 transition-all">Keluar</button>
      </nav>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 space-y-8">
          ...
          {/* Form Input Materi (Sama seperti sebelumnya) */}
          <div className="bg-white border-4 border-slate-900 p-6 rounded-3xl shadow-[8px_8px_0_0_rgba(15,23,42,1)]">
            <h2 className="text-xl font-black text-slate-900 mb-6 border-b-4 border-slate-900 pb-2">➕ Tambah Materi Baru</h2>
            <form onSubmit={simpanMateri} className="space-y-5">
              <div>
                <label className="block font-bold text-slate-900 mb-2">Judul</label>
                <input type="text" value={judul} onChange={(e) => setJudul(e.target.value)} className="w-full p-3 rounded-xl border-4 border-slate-900 bg-slate-50 focus:bg-white font-bold" />
              </div>
              <div>
                <label className="block font-bold text-slate-900 mb-2">Mata Pelajaran</label>
                <select value={mapel} onChange={(e) => setMapel(e.target.value)} className="w-full p-3 rounded-xl border-4 border-slate-900 bg-slate-50 focus:bg-white font-bold cursor-pointer">
                  <option>Aljabar</option><option>Kalkulus</option><option>Trigonometri</option><option>Statistika</option>
                </select>
              </div>
              <div>
                <label className="block font-bold text-slate-900 mb-2">Format</label>
              ...
              <div>
                <label className="block font-bold text-slate-900 mb-2">Link</label>
                <input type="url" value={link} onChange={(e) => setLink(e.target.value)} className="w-full p-3 rounded-xl border-4 border-slate-900 bg-slate-50 focus:bg-white font-bold" />
              </div>
              <button type="submit" disabled={loading} className={`w-full text-slate-900 text-lg font-black py-4 rounded-xl border-4 border-slate-900 shadow-[6px_6px_0_0_rgba(15,23,42,1)] active:shadow-none active:translate-y-1 active:translate-x-1 uppercase ${loading ? 'bg-slate-300' : 'bg-blue-400 hover:bg-blue-500'}`}>
                Simpan Materi
              </button>
            </form>
          </div>
            ...
            {materiList.map((materi) => (
              <div key={materi.id} className={`flex items-center justify-between border-4 border-slate-900 p-4 rounded-2xl ${materi.format === 'Video' ? 'bg-yellow-100' : materi.format === 'Artikel' ? 'bg-blue-100' : 'bg-orange-100'}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div><h3 className="font-black text-slate-900 text-lg">{materi.judul}</h3><p className="text-sm font-bold text-slate-600">{materi.mapel} • {materi.format}</p></div>
                </div>
