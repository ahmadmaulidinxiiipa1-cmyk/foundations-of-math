...
export default function DasborPage() {
  ...
  // Fungsi untuk mengarahkan siswa ke Halaman Metode beserta membawa data Jenjang
  const pilihJenjang = (jenjang: string) => {
    // Kita selipkan pilihan jenjangnya ke URL agar tidak hilang (contoh: /metode?jenjang=SMA)
    router.push(`/metode?jenjang=${jenjang}`);
  };

  return (
    <main className="min-h-screen bg-green-50 p-4 md:p-6 relative overflow-hidden font-sans flex flex-col items-center justify-center">
      ...
      {/* Kotak Utama */}
      <div className="bg-white border-4 md:border-8 border-slate-900 p-6 md:p-10 rounded-3xl md:rounded-[3rem] w-full max-w-md md:max-w-2xl text-center shadow-[8px_8px_0_0_rgba(15,23,42,1)] md:shadow-[12px_12px_0_0_rgba(15,23,42,1)] z-10 relative">
